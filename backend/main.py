    ...
    for index, row in df_c.iterrows():
        ...
        if not kit_row.empty:
            ...
            if not facturas_cliente.empty:
                nuevo_estado = "Facturas lanzadas"
                if pd.isna(row['Estado_Nofence']) or str(row['Estado_Nofence']).strip() == "":
                    df_c.at[index, 'Estado_Nofence'] = "Pendiente de avisar"
            
            if not facturas_cliente.empty:
                pagadas = facturas_cliente[facturas_cliente['Estado_Pago'] == 'Pagado']
                if len(pagadas) == len(facturas_cliente):
